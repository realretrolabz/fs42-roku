...
BRIDGE_SERVICE_FILE="$SYSTEMD_USER_DIR/fs42-hls-bridge.service"
...
FS42_BRIDGE_CONFIG="$FS42_DIR/hls_bridge.env"
FS42_BRIDGE_RUNNER="$FS42_DIR/run_hls_bridge.sh"
...
load_existing_bridge_service_config() {
    if [ ! -f "$BRIDGE_SERVICE_FILE" ]; then
        return
    fi

    local exec_start
    exec_start="$(awk '/^ExecStart=/ {sub(/^ExecStart=/, ""); print; exit}' "$BRIDGE_SERVICE_FILE")"
    if [ -z "$exec_start" ]; then
        return
    fi

    # The service generated by this installer uses simple whitespace-separated args.
    # Reading it back keeps reruns from silently forgetting the previous capture mode.
    local parts
    read -r -a parts <<< "$exec_start"
    local i=0
    while [ "$i" -lt "${#parts[@]}" ]; do
        case "${parts[$i]}" in
            --status-url)
                FS42_STATUS_URL="${parts[$((i + 1))]:-$FS42_STATUS_URL}"
                i=$((i + 1))
                ;;
            --hls-dir)
                HLS_DIR="${parts[$((i + 1))]:-$HLS_DIR}"
                i=$((i + 1))
                ;;
            --public-host)
                PUBLIC_HOST="${parts[$((i + 1))]:-$PUBLIC_HOST}"
                i=$((i + 1))
                ;;
            --port)
                BRIDGE_PORT="${parts[$((i + 1))]:-$BRIDGE_PORT}"
                i=$((i + 1))
                ;;
            --display)
                DISPLAY_VALUE="${parts[$((i + 1))]:-$DISPLAY_VALUE}"
                i=$((i + 1))
                ;;
            --capture-size)
                CAPTURE_SIZE="${parts[$((i + 1))]:-$CAPTURE_SIZE}"
                i=$((i + 1))
                ;;
            --capture-offset)
                CAPTURE_OFFSET="${parts[$((i + 1))]:-$CAPTURE_OFFSET}"
                i=$((i + 1))
                ;;
            --capture-framerate)
                CAPTURE_FRAMERATE="${parts[$((i + 1))]:-$CAPTURE_FRAMERATE}"
                i=$((i + 1))
                ;;
            --full-frame-capture-size)
                FULL_FRAME_CAPTURE_SIZE="${parts[$((i + 1))]:-$FULL_FRAME_CAPTURE_SIZE}"
                i=$((i + 1))
                ;;
            --full-frame-capture-offset)
                FULL_FRAME_CAPTURE_OFFSET="${parts[$((i + 1))]:-$FULL_FRAME_CAPTURE_OFFSET}"
                i=$((i + 1))
                ;;
            --full-frame-content-types)
                FULL_FRAME_CONTENT_TYPES="${parts[$((i + 1))]:-$FULL_FRAME_CONTENT_TYPES}"
                i=$((i + 1))
                ;;
            --audio-source)
                AUDIO_SOURCE="${parts[$((i + 1))]:-$AUDIO_SOURCE}"
                i=$((i + 1))
                ;;
            --pulse-source)
                PULSE_SOURCE="${parts[$((i + 1))]:-$PULSE_SOURCE}"
                i=$((i + 1))
                ;;
        esac
        i=$((i + 1))
...
load_existing_bridge_env_config() {
    if [ ! -f "$FS42_BRIDGE_CONFIG" ]; then
        return
    fi
    ...
    while IFS='=' read -r key value; do
        case "$key" in
            FS42_STATUS_URL)
                FS42_STATUS_URL="${value:-$FS42_STATUS_URL}"
                ;;
            HLS_DIR)
                HLS_DIR="${value:-$HLS_DIR}"
                ;;
            PUBLIC_HOST)
            ...
            CAPTURE_SIZE)
                ...
                ;;
            CAPTURE_OFFSET)
                CAPTURE_OFFSET="${value:-$CAPTURE_OFFSET}"
                ;;
            CAPTURE_FRAMERATE)
                CAPTURE_FRAMERATE="${value:-$CAPTURE_FRAMERATE}"
                ;;
            FULL_FRAME_CAPTURE_SIZE)
                FULL_FRAME_CAPTURE_SIZE="${value:-$FULL_FRAME_CAPTURE_SIZE}"
                ;;
            FULL_FRAME_CAPTURE_OFFSET)
                FULL_FRAME_CAPTURE_OFFSET="${value:-$FULL_FRAME_CAPTURE_OFFSET}"
                ;;
            FULL_FRAME_CONTENT_TYPES)
                FULL_FRAME_CONTENT_TYPES="${value:-$FULL_FRAME_CONTENT_TYPES}"
                ;;
            AUDIO_SOURCE)
                AUDIO_SOURCE="${value:-$AUDIO_SOURCE}"
                ;;
            PULSE_SOURCE)
                PULSE_SOURCE="${value:-$PULSE_SOURCE}"
                ;;
        esac
    done < "$FS42_BRIDGE_CONFIG"
}

write_bridge_env_config() {
    cat > "$FS42_BRIDGE_CONFIG" <<CONFIG
FS42_STATUS_URL=$FS42_STATUS_URL
HLS_DIR=$HLS_DIR
...
FULL_FRAME_CAPTURE_SIZE=$FULL_FRAME_CAPTURE_SIZE
FULL_FRAME_CAPTURE_OFFSET=$FULL_FRAME_CAPTURE_OFFSET
FULL_FRAME_CONTENT_TYPES=$FULL_FRAME_CONTENT_TYPES
...
write_bridge_runner() {
    local full_frame_args=""
    if [ -n "$FULL_FRAME_CAPTURE_SIZE" ]; then
        full_frame_args=" --full-frame-capture-size \"\$FULL_FRAME_CAPTURE_SIZE\" --full-frame-capture-offset \"\$FULL_FRAME_CAPTURE_OFFSET\" --full-frame-content-types \"\$FULL_FRAME_CONTENT_TYPES\""
    fi

    cat > "$FS42_BRIDGE_RUNNER" <<RUNNER
#!/usr/bin/env bash

set -euo pipefail

SCRIPT_DIR="\$(cd "\$(dirname "\${BASH_SOURCE[0]}")" && pwd)"
source "\$SCRIPT_DIR/hls_bridge.env"

exec /usr/bin/env python3 "\$SCRIPT_DIR/hls_bridge.py" \\
  --status-url "\$FS42_STATUS_URL" \\
  --hls-dir "\$HLS_DIR" \\
  ...
  --capture-framerate "\$CAPTURE_FRAMERATE"$full_frame_args \\
  --audio-source "\$AUDIO_SOURCE" \\
  --pulse-source "\$PULSE_SOURCE"
RUNNER
    chmod +x "$FS42_BRIDGE_RUNNER"
}

is_480p_optimized_config() {
...
if [ -f "$BRIDGE_SERVICE_FILE" ]; then
    echo "  Existing bridge service detected; using its settings as defaults."
elif [ -f "$FS42_BRIDGE_CONFIG" ]; then
    echo "  Existing bridge config detected; using its settings as defaults."
fi
echo ""
...
OPTIMIZED_DEFAULT="N"
if is_480p_optimized_config; then
    OPTIMIZED_DEFAULT="Y"
fi
if yes_no "Enable 480p optimized capture mode?" "$OPTIMIZED_DEFAULT"; then
    CAPTURE_SIZE="640x480"
    CAPTURE_OFFSET="40,0"
    FULL_FRAME_CAPTURE_SIZE="720x480"
    FULL_FRAME_CAPTURE_OFFSET="0,0"
...
if [ "$INSTALL_BRIDGE_SERVICE" = true ]; then
    ...
    cat > "$BRIDGE_SERVICE_FILE" <<SERVICE
[Unit]
...
echo "Selected bridge capture settings:"
echo "  default:   $CAPTURE_SIZE at offset $CAPTURE_OFFSET"
if [ -n "$FULL_FRAME_CAPTURE_SIZE" ]; then
    echo "  full-frame: $FULL_FRAME_CAPTURE_SIZE at offset $FULL_FRAME_CAPTURE_OFFSET for $FULL_FRAME_CONTENT_TYPES"
else
    echo "  full-frame: disabled"
fi
echo ""
...
echo "Wrote bridge config:"
echo "  $FS42_BRIDGE_CONFIG"
echo "Wrote non-systemd runner:"
echo "  $FS42_BRIDGE_RUNNER"
...
if [ "${#INSTALLED_SERVICES[@]}" -gt 0 ]; then
    ...
    if [ "$INSTALL_BRIDGE_SERVICE" = true ] && [ "$ENABLE_SELECTED" != true ]; then
        echo ""
        echo "The bridge service file was updated, but the running service was not restarted."
        echo "Run this to apply the new capture settings:"
...
elif [ "$INSTALL_BRIDGE_SERVICE" != true ]; then
    echo ""
    echo "Bridge service was not installed/updated. Use this runner to apply the saved settings:"
    echo "  $FS42_BRIDGE_RUNNER"
fi
